...
**Ask before adding one.** When the screen is one where an illustration earns its place (onboarding, empty/success state, permission priming) and the brief hasn't already specified one, ask the developer up front — before building — two things in order: (1) whether to include an illustration here at all, and only if yes, (2) which source to use: their own asset set, a free vector library (unDraw / Open Peeps / Lottie), or AI-generated. Then read `references/illustrations.md` and keep every illustration consistent with that answer for the rest of the app. Don't ask on screens where illustrations don't belong (settings, forms, dense lists, data tables) — there, just proceed. If the developer has already named a source, skip the questions and use it.
